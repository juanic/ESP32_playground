 ...
 * @file touch_hal.c
 * @author Juan Cerrudo
 * @brief Touch pad driver for ESP32-S3 (touch sensor hardware version 2).
 * @version 0.1
 * @date 2026-08-24
 *
...
#define TOUCH_PAD_QTY 	TOUCH_TOTAL_CHAN_NUM
...
bool TouchHalInit(void){
	...
	/* Default sampling for touch sensor v2: charge/discharge 100 times,
	 * 0.5V..2.2V charge/discharge range. */
	s_sample_cfg = (touch_sensor_sample_config_t)TOUCH_SENSOR_V2_DEFAULT_SAMPLE_CONFIG(
		100, TOUCH_VOLT_LIM_L_0V5, TOUCH_VOLT_LIM_H_2V2);
...
bool TouchHalIsTouched(touch_t pad){
	...
	/* On the ESP32-S3 (touch v2) the channel value INCREASES when touched,
	 * so a channel is considered touched when its value is above the threshold. */
