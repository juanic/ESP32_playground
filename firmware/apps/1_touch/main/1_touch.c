 ...
 *
 * This example reads two capacitive touch pads of the ESP32 (touch_hal) and
 ...
 * useful to tune CONFIG_TOUCH_THRESHOLD for your own wiring/finger.
 *
...
/* Adjust these thresholds to your own wiring: on boot, log the untouched
 * value with a finger away from the pad and set the threshold to ~2/3 of it */
...
void app_main(void){
    ...
    while(true){
        ...
        if(TouchHalRead(TOUCH_BTN_1, &value_1) && TouchHalRead(TOUCH_BTN_2, &value_2)){
            printf("T8 (GPIO33): %4" PRIu32 " %s | T9 (GPIO32): %4" PRIu32 " %s\n",
